...
# Analagous functions as those used in the ABC module 'ABC_Res_Pop_Model'.
...
# Custom function to just keep items 2 and 3 (population sizes) of the solution,
# item 4 (passage ID).  
...
function grow_kill_fxn(n0::Int64, nS::Float64, nR::Float64, nE::Float64,
    ...
    u0 = [0.0, nS, nR, nE, 1]      # u[5] = Passage number # g = 0.0 | t = 0.0
    tspan = (t0, tmax)
...
function exp_pulse_treat_fxn(n0::Int64, b::Float64, d::Float64,
    ...
    Dc::Float64, k::Float64, psi::Float64,
    t_exp::Float64, tmax::Float64, t_Pass::Float64, Nmax::Int64, Cc::Int64,
    treat_ons::Array{Float64}, treat_offs::Array{Float64}, 
...
# A separate version of the function that returns the full population outputs 
# (sensitive, resistant and escape phenotypes, and total population size).

function exp_pulse_treat_full_sol_fxn(n0::Int64, b::Float64, d::Float64,
    rho::Float64, mu::Float64, sig::Float64, del::Float64, al::Float64,
    Dc::Float64, k::Float64, psi::Float64,
    ...
    if (exp_nS + exp_nR + exp_nE) < n0*4
        ...
        for i in 1:4
            ...
            sol_df = DataFrame(
                ...
                nR = map(x -> x[3], sol.u),
                nE = map(x -> x[4], sol.u),
                N = map(x -> x[2], sol.u) .+ map(x -> x[3], sol.u) .+ map(x -> x[4], sol.u),
                rep = i
            )
        ...
        fin_sol_df = vcat(sol_dfs...)

        # Return the vectors of t and u
        return Dict("t"=>fin_t_outs, "u"=>fin_u_outs, "sol_df"=>fin_sol_df)
